...
if (resizing_state == "width") {
	if (target_arena_width != arena_width) {
		if (target_arena_width > arena_width) { // arena_width needs to increase
			if (arena_width + arena_speed > target_arena_width) {
				arena_width = target_arena_width; // always off by 1 for some reason
			} else {
				arena_width += arena_speed;
			}
		} else if (target_arena_width < arena_width) { // arena_width needs to decrease
			if (arena_width - arena_speed < target_arena_width) {
				arena_width = target_arena_width; // again
			} else {
				arena_width -= arena_speed;
			}
		}
	} else {
		if (target_arena_height <= arena_height and target_arena_width <= arena_width) {
			resizing_state = "DONE";
		} else {
...
if (resizing_state == "height") {
	if (target_arena_height != arena_height) {
		if (target_arena_height > arena_height) { // arena_height needs to increase
			if (arena_height + arena_speed > target_arena_height) {
				arena_height = target_arena_height;
			} else {
				arena_height += arena_speed;
			}
		} else if (target_arena_height < arena_height) { // arena_height needs to decrease
			if (arena_height - arena_speed < target_arena_height) {
				arena_height = target_arena_height;
			} else {
				arena_height -= arena_speed;
...
// x for half speed
if (keyboard_check(ord("X")) == 1 or keyboard_check(vk_shift) == 1) {
	soul_speed = 2.5;
} else {
	soul_speed = 5;
}

// can we do more comments because wwwwhat is this supposed to do 6~6
// nevermind i with my infinite knowledge and understanding have remembered what this does
arena_h1 = arena_width / 2;
arena_h2 = arena_width / 2;

arena_v1 = arena_height / 2;
arena_v2 = arena_height / 2;

bottom_wall_x = round(arena_center_x - arena_h1) - 5;
bottom_wall_y = floor(arena_center_y + arena_v2); // Floored because of some weird scaling issues

top_wall_x = round(arena_center_x - arena_h1) - 5;
top_wall_y = round(arena_center_y - arena_v2) - 5;

left_wall_x = round(arena_center_x - arena_h1) - 5;
left_wall_y = top_wall_y;

right_wall_x = round(arena_center_x + arena_h2);
right_wall_y = top_wall_y;

if (keyboard_check(ord("Q"))) {
	krCon.ApplyKR(1);
	audio_play_sound(playerhit, 1, 0);
	show_debug_message("Applied 1 KR.");
}
...
if (global.player_hp == 0) { // LOOKS [[Like And Subscribe]] Y OU'RE NOT A [[BIG SHOT!!!]] AN YMORE.
	...
	gh1999.soul_x = soul_x;
	gh1999.soul_y = soul_y;
	gh1999.textMgr = textMgr;
	textMgr.clear_all_text();
	room_goto(room_gameover);
}

if (state == "buttonselect" and resizing_state == "DONE") {
	sel_btnb = sel_btn;
	...
	fightbt_spr = FIGHTs_1;	
	actbt_spr = ACTs_1;
	itembt_spr = ITEMs_1;
	mercybt_spr = MERCYs_1;
	...
	if (has_flavor_text == false) {
		// function(tx, ty, textSpd, font, active, text)
		flavor_text_index = textMgr.createText(left_wall_x + 20, top_wall_y + 25, 30, dtf, true, flavortext);
...
if (state == "fightmenu") {
	...
	if (fight_on) {
		...
		soul_x = left_wall_x + 32;
		soul_y = top_wall_y + 28;
		
		fight_1 = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 25, dtf, true, enemy_name);
		fight_on = false;
	}
...
if (state == "actmenu") {
	...
	if (act_on) {
		// Initialize ACT menu
		soul_x = left_wall_x + 32; // todo: double check this is right
		soul_y = top_wall_y + 28;
		...
		if (can_check) {
			act_check = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 25, dtf, true, "Check");
		}
		
		// Multiple ACTs example
		if (total_acts >= 2) {
			act_2_label = textMgr.createTextInstant(left_wall_x + 324, top_wall_y + 25, dtf, true, "Talk");
		}
		
		if (total_acts >= 3) {
			act_3_label = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 57, dtf, true, "Apologize");
		}
		
		if (total_acts == 4) {
			act_4_label = textMgr.createTextInstant(left_wall_x + 324, top_wall_y + 57, dtf, true, "Taunt");
		}
	...
	if (sel_sub != sel_subb) {
		...
		if (sel_sub == 2) {
			soul_x = left_wall_x + 292; // todo: double check this is right
			soul_y = top_wall_y + 28;
		}
		if (sel_sub == 3) {
			soul_x = left_wall_x + 32; // todo: double check this is right
			soul_y = top_wall_y + 60;
		}
		if (sel_sub == 4) {
			soul_x = left_wall_x + 292; // todo: double check this is right
			soul_y = top_wall_y + 60;
...
if (state == "checking") {
	if (check_on) {
		check_text_index = textMgr.createText(left_wall_x + 20, top_wall_y + 25, 30, dtf, true, check_msg);
		check_on = false;
	}
...
if (state == "actmenu2") {
	if (act2_on) {
		act2_text_index = textMgr.createText(left_wall_x + 20, top_wall_y + 25, 30, dtf, true, "you got exactly what\nyou want!\n2");
		act2_on = false;
	}
...
if (state == "actmenu3") {
	if (act3_on) {
		act3_text_index = textMgr.createText(left_wall_x + 20, top_wall_y + 25, 30, dtf, true, "i got exactly who\ni wanna be\n3");
		act3_on = false;
	}
...
if (state == "actmenu4") {
	if (act4_on) {
		act4_text_index = textMgr.createText(left_wall_x + 20, top_wall_y + 25, 30, dtf, true, "i was inspired not\nto [HYPERLINK BLOCKED]\n4");
		act4_on = false;
	}
...
if (state == "itemmenu") {
	if (array_length(items) != 0) {
		...
		if (item_on) {
			...
			if (page == 1) {
				...
				if (array_length(items) >= 1) {
					item_1_label = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 25, dtf, true, items[0]);
				}
		
				if (array_length(items) >= 2) {
					item_2_label = textMgr.createTextInstant(left_wall_x + 308, top_wall_y + 25, dtf, true, items[1]);
				}
		
				if (array_length(items) >= 3) {
					item_3_label = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 57, dtf, true, items[2]);
				}
		
				if (array_length(items) >= 4) {
					item_4_label = textMgr.createTextInstant(left_wall_x + 308, top_wall_y + 57, dtf, true, items[3]);
				}
				
				page_label = textMgr.createTextInstant(left_wall_x + 356, top_wall_y + 85, dtf, true, "PAGE 1", false);
			}
			
			if (page == 2) {
				if (array_length(items) >= 5) {
					item_1_label = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 25, dtf, true, items[4]);
				}
		
				if (array_length(items) >= 6) {
					item_2_label = textMgr.createTextInstant(left_wall_x + 308, top_wall_y + 25, dtf, true, items[5]);
				}
		
				if (array_length(items) >= 7) {
					item_3_label = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 57, dtf, true, items[6]);
				}
		
				if (array_length(items) == 8) {
					item_4_label = textMgr.createTextInstant(left_wall_x + 308, top_wall_y + 57, dtf, true, items[7]);
				}
				
				page_label = textMgr.createTextInstant(left_wall_x + 356, top_wall_y + 85, dtf, true, "PAGE 2", false);
			}
		...
		if (sel_sub == 2) {
			soul_x = left_wall_x + 280; // todo: double check this is right
			soul_y = top_wall_y + 28;
		}
		if (sel_sub == 3) {
			soul_x = left_wall_x + 32; // todo: double check this is right
...
if (state == "consumingItem") {
	if (csi_on) {
		...
		csi_text_index = textMgr.createText(left_wall_x + 20, top_wall_y + 25, 30, dtf, true, item_flavor_text);
		csi_on = false;
	}
...
if (state == "mercymenu") {
	...
	if (mercy_on) {
		spare_label = textMgr.createTextInstant(left_wall_x + 68, top_wall_y + 25, dtf, true, "Spare");
		soul_x = left_wall_x + 32; // todo: double check this is right
		soul_y = top_wall_y + 28;
		mercy_on = false;
	}
...
if (state == "attacking") {
	...
	if ((slicer_x >= right_wall_x + 5 and fromSide == 1) or (slicer_x <= left_wall_x - sprite_get_width(atk_slicer) and fromSide == 0)) {
		sspeed = 0;
		sliced = true;
...
if (state == "monster_dialog") {
	soul_x = floor(arena_center_x) - floor(sprite_get_width(SOULs) / 2);
	soul_y = floor(arena_center_y) - floor(sprite_get_height(SOULs) / 2);
